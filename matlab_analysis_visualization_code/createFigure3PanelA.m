function [] = createFigure3PanelA(assemSize)
...
assemSet = ismember(SRPyrRaster(2,:),nonAssem);
assemIdx = find(assemSet);
C = setdiff([1:1:size(SRPyrRaster,2)],[assemIdx]);

fh = figure; clf; plot(SRPyrRaster(1,C), SRPyrRaster(2,C), '.k','MarkerSize', mSize);
hold on; 
plot(SRPyrRaster(1,assemIdx), SRPyrRaster(2,assemIdx), '.r','MarkerSize', mSize);
...
xlabel('time (s)');
ylabel('Neuron #');
title('Network Raster Activity During Training of Three Patterns')
xlim([45000 46000])
ax = gca;
ax.LineWidth = 5;
box off;
